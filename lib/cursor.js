...
function parseDailyUsage(response) {
  ...
  for (const record of response.data || []) {
    ...
    if (record.isActive) activeUserEmails.add(email);
    ...
    if (dateKey !== 'unknown') {
      if (!byDate[dateKey]) {
        byDate[dateKey] = { linesAdded: 0, acceptedLines: 0, activeUsers: new Set() };
      }
      byDate[dateKey].linesAdded += record.totalLinesAdded || 0;
      byDate[dateKey].acceptedLines += record.acceptedLinesAdded || 0;
      if (record.isActive) byDate[dateKey].activeUsers.add(email);
    }
  }

  // Calculate per-user percentages
  for (const email of Object.keys(byUser)) {
    const user = byUser[email];
    user.aiCodePercent = user.totalLinesAdded > 0
      ? (user.acceptedLinesAdded / user.totalLinesAdded) * 100
      : 0;
    user.tabAcceptRate = user.totalTabsShown > 0
      ? (user.totalTabsAccepted / user.totalTabsShown) * 100
      : 0;
  }

  // Convert byDate activeUsers Sets to counts
  for (const dateKey of Object.keys(byDate)) {
    byDate[dateKey].activeUsers = byDate[dateKey].activeUsers.size;
  }

  // Calculate totals
  totals.activeUsers = activeUserEmails.size;
...
function parseSpend(response) {
  ...
  let totalIncludedSpendCents = 0;

  for (const member of response.teamMemberSpend || []) {
    const email = member.email || 'unknown';
    totalSpendCents += member.spendCents || 0;
    totalIncludedSpendCents += member.includedSpendCents || 0;

    byUser[email] = {
      spendCents: member.spendCents || 0,
      spendDollars: (member.spendCents || 0) / 100,
      includedSpendCents: member.includedSpendCents || 0,
      includedSpendDollars: (member.includedSpendCents || 0) / 100,
  ...
  return {
    ...
    totalIncludedSpendCents,
    totalIncludedSpendDollars: totalIncludedSpendCents / 100,
    totalUsageDollars: (totalSpendCents + totalIncludedSpendCents) / 100,
    byUser
  };
}
